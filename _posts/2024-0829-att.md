...
<!-- refers to the action of providing a statement or declaration about the truth, correctness, or validity of something. In the context of technology and security, attestation often involves the verification of the **integrity** and **authenticity** of data or processes.
Typically, attestation is issued by an attestor or issuer to the prover (user) after the prover successfully demonstrates their qualification. -->

<!-- In both scenarios, a proof or attestation will be verified by another party called verifier for specific applications and use cases. Sometimes, you can prove things directly to verifiers without any extra steps. But, there are situations, especially in the Web3 projects, where this isn't enough. That's when we bring in attestation. It's like getting an official stamp on your proof to make sure everything is trustworthy.  -->
...
<!-- As demonstrated in the above example, attestation emerges as a crucial concept, serving as a key bridge between distinct and independent systems, particularly for cross-domain applications. This attribute makes it particularly suitable for applications that between Web2 and Web3.

### 2.1 Bring Web2 data to Web3
In the ever-evolving landscape of the internet, the intersection of Web2 and Web3 technologies has sparked a transformative shift—one that is rooted in the seamless integration of data. The significance of bringing Web2 data into the Web3 ecosystem goes beyond technological evolution; it unlocks a data-driven revolution that holds immense potential for innovation, user empowerment, and the future of a decentralized internet.

Web2 platforms have amassed a wealth of diverse data through years of user interactions. Integrating this abundance of information into the Web3 ecosystem empowers developers to craft innovative solutions that capitalize on the strengths of both centralized and decentralized technologies. With decentralized technologies, users gain the ability to own, selectively share, and compute their data in a privacy-preserving manner. This aligns seamlessly with the escalating demand for heightened privacy and data ownership rights, placing users in control of their digital experiences in a secure and transparent environment. -->
...
With this signature, a user can prove the receipt of an email from a specific sender, say X, by sharing the email header from name@X.com to you@domain.com. Anyone can fetch the public key of X and use it to verifiy the signature.  Since the email header contains a hash of the email body, it becomes possible to prove the content of the email by sharing the email body. The public key holder can then verify the content.

To enhace security and reduce the cost of on-chain verification. The user can then apply zero-knowledge proofs to prove the authenticity of the email and content inside.
...
The trust assumptions are simlar to the DKIM case. They rely on the trust of the OpenID provider and the secure means of fetching a public key.


### 3.3 Integerating TLS for General Web Data
...
<!-- Theoretically, an extreme case could involve a malicious user attempting to cheat. It's crucial to emphasize that this "attack" is exceptionally challenging to execute in real-world applications. **This note is provided in the spirit of thorough consideration, and it is not intended to cast doubt on the validity of the solution.**

<div align=center>
    <img src = "https://hackmd.io/_uploads/B1CEdla9p.png" style="zoom:55%">
    
</div>

In this extreme scenario, a malicious user could potentially position himself between the attestor and the TLS Server, after the establishment of the session key by the TLS Client and Server. In such a case, the malicious user might have the capability to fabricate any information, given the knowledge of the session key. **It's essential to underscore that this is an extremely rare and difficult situation for a malicious user to exploit, as it involves taking control of the channel and impersonating the TLS Server within a very short period of time.** -->
...
To further minimize reliance on attestors and promote decentralization, various attestation frameworks have been introduced , such as [EAS](https://attest.sh/), [Verax](https://www.ver.ax/). These frameworks provide generic methods, allowing anyone to register as an attestor within the network, regardless of the attestation method used. This approach offers flexibility for users and applications to choose any attestor and attestation method based on their specific requirements.
<!-- [BAS](https://medium.com/@Rido_io/first-interpretation-of-bas-805560f1daf0),  --> 
